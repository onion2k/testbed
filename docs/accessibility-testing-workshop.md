...
This workshop connects accessibility testing with both product quality and automation quality. The aim is to give a manual tester enough context to feel oriented rather than overwhelmed. A lot of people arrive in automation expecting a sharp break with their earlier skills. In practice, the transition works best when you can see how the habits you already trust connect to a more repeatable and more technical style of testing. This workshop takes that approach from the beginning by slowing down and making the reasoning visible.

As you read, keep in mind that the goal is not to turn you into a framework specialist in one sitting. The goal is to help you build a mental model of how accessible names, focus behavior, keyboard support, and semantic structure improve the experience for users and the reliability of tests. Once that mental model is steady, the tools become easier to learn because they have a clear purpose. The workshop is therefore written in plain technical language and assumes you are capable, curious, and ready to connect new techniques to the quality instincts you already use every day.

## Why This Matters

How accessible names, focus behavior, keyboard support, and semantic structure improve the experience for users and the reliability of tests matters because automation becomes expensive very quickly when it is built on vague thinking. A tester who understands the purpose of a check, the evidence it needs, and the risk it is trying to control will usually produce stronger automation than someone who starts with syntax and hopes the value will appear later. That is why this workshop spends time on explanation rather than only on instructions. The explanation is what lets you transfer the lesson to other products later.
...
In Testbed, you can explore checking labels, focus movement, keyboard interaction, and visible feedback across the shop and checkout journeys while noting how those same choices improve Playwright locators. That makes the lesson unusually practical because the environment is designed to let you move between visible behavior and the hidden state behind it. The browser experience gives you the product journey. The desktop shell gives you controlled setup and observation. The API gives you a direct view of the messages the browser depends on. Those three layers make abstract ideas much easier to understand.
...
```quiz
id: a11y-meaning-check
question: Why does accessibility matter to automation as well as users?
passCondition: all
options:
  - id: option-1
    label: Clear names, roles, and focus behavior improve both usability and locator quality
    correct: true
  - id: option-2
    label: Accessible interfaces remove the need for testing
    correct: false
  - id: option-3
    label: Accessibility only matters to specialist auditors, not normal testers
    correct: false
```

## A Simple Example

A simple way to make this workshop concrete is to imagine testing a form field with keyboard navigation, confirming that focus is visible and labels are meaningful, and then using the same accessible label in an automated locator. At first this may seem like a straightforward product task, but that simplicity is useful. It gives you a small, familiar surface to think with. The important question is not only whether the action succeeds. The important question is what you would want to know, what evidence you would keep, and which part of the system would be the clearest place to check it.
...
Accessibility work often feels bigger than it really is because people imagine it only in terms of formal standards. Those standards matter, but everyday accessibility testing usually begins with simpler questions. Can a user find and understand controls? Does focus move predictably? Is the structure of the page meaningful to assistive technology? Can actions be completed without a mouse? Those are all ordinary testing questions once you start asking them regularly. They are also closely related to automation because the same semantic quality that helps users often helps tools locate elements reliably.

Testbed makes this easier to see because the product flows are small enough to inspect carefully. A sign-in form with clear labels is easier to use and easier to automate. A dialog with predictable focus behavior is easier to test manually and easier to script. This is one of the reasons strong accessibility habits often produce stronger automation suites as a side effect. They encourage interfaces that expose stable meaning instead of relying on incidental structure.

## How To Practise This Well

A good practice session starts with one journey completed entirely by keyboard. Do not rush. Notice which controls are reachable, whether focus is visible, and whether the order of movement makes sense. Then compare what you observed with the locators you would choose in Playwright. If an element is hard for you to understand through the UI, it is often hard for a test to find meaningfully as well.

Over time, you want accessibility to become part of ordinary test design rather than a separate box you tick occasionally. That means noticing names, roles, focus, and feedback while you already test other behaviors. When those habits combine, quality improves on more than one front at once.

```quiz
id: a11y-practice-check
question: What is a good practical starting exercise for accessibility testing?
passCondition: all
options:
  - id: option-1
    label: Run a journey by keyboard and observe focus, labels, and feedback carefully
    correct: true
  - id: option-2
    label: Only test visual styling with the mouse
    correct: false
  - id: option-3
    label: Avoid automation locators while checking accessibility
    correct: false
```

## Common Beginner Mistake

The most common beginner mistake in this area is treating accessibility as a specialist topic that sits outside normal testing rather than as part of everyday product quality. The problem with that habit is not only that it leads to weaker tests. It also makes learning slower. When you misread the purpose of the activity, every tool feels more confusing because you are asking it to solve the wrong problem.
...
In this workshop, good practice looks like building accessibility checks into ordinary journeys so usability, inclusion, and testability improve together. It is usually calmer and more deliberate than beginners expect. You are not trying to cover everything at once. You are trying to choose the next most useful thing to understand, check, or improve.
...
```quiz
id: a11y-final-check
question: What does strong accessibility practice look like?
passCondition: all
options:
  - id: option-1
    label: Treating accessibility as part of everyday product quality, not as a separate afterthought
    correct: true
  - id: option-2
    label: Leaving accessibility entirely to external audits
    correct: false
  - id: option-3
    label: Checking it only when automation is already broken
    correct: false
```
